...
struct Test1 {
  ...
  static void OnCaptureCompare1() { CCTimer::Stop(); }

  static void OnCaptureCompare2() { CCTimer::Stop(); }
...
class Application {
 ...
 public:
  ...
  using AppHardwareTimer = HardwareTimerBase<TIM2, TimerCountable,
                                             InterruptsList<>>;  // Прикручиваем этот список в аппаратному таймеру,
                                                                 // который будет использоваться для расчета частоты
  using AppHardwareTimer1 = HardwareTimerBase<TIM1, TimerCcpableInterruptable,
                                              InterruptsList<>>;  // Прикручиваем этот список в аппаратному таймеру,
                                                                  // который будет использоваться для расчета частоты
  using AppCCHardwareTimer = HardwareCCTimerBase<AppHardwareTimer1, InterruptsList<CaptureTimer1,
                                                                                   CaptureTimer2>>;  // Прикручиваем этот список в аппаратному
                                                                                                     // таймеру, который будет использоваться
                                                                                                     // для расчета частоты

  struct DurationTimer : HardwareOverflowTimer<AppHardwareTimer, OverflowObservers<Test>> {};

  struct CaptureTimer1 : HardwareCCxTimerBase<AppHardwareTimer1, CcTimer1Observers<Test, Test1<DurationTimer, CaptureTimer1>>, CC1> {};

  struct CaptureTimer2 : HardwareCCxTimerBase<AppHardwareTimer, CcTimer2Observers<Test1<Test, CaptureTimer2>>, CC2> {};
  ...
  struct HardwareUart : HardwareUartBase<USART2, UartTxInterruptable,
                                         InterruptsList<HardwareUartTx<HardwareUart, UartTransmitObservers<MyDriver>>,
