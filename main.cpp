...
#include "hardwaretimeroverflow.hpp"
...
constexpr std::uint32_t UartSpeed9600 =
    static_cast<std::uint32_t>(SysClock / 9600U);
...
int __low_level_init(void) {
  ...
  RCC::AHB1ENRPack<RCC::AHB1ENR::GPIOAEN::Enable>::Set();

  // Switch on clock on PortA and PortC, PortB
  RCC::AHB1ENRPack<RCC::AHB1ENR::GPIOCEN::Enable, RCC::AHB1ENR::GPIOAEN::Enable,
                   RCC::AHB1ENR::GPIOBEN::Enable>::Set();

  using Apb1Enaler = RCC::APB1ENRPack<
      RCC::APB1ENR::TIM5EN::Enable, RCC::APB1ENR::TIM2EN::Enable,
      RCC::APB1ENR::SPI2EN::Enable, RCC::APB1ENR::USART2EN::Enable>;
  RCC::APB1ENRPack<RCC::APB1ENR::TIM5EN::Enable, RCC::APB1ENR::TIM2EN::Enable,
                   RCC::APB1ENR::SPI2EN::Enable,
                   RCC::APB1ENR::USART2EN::Enable>::Write();

  GPIOA::MODERPack<GPIOA::MODER::MODER5::Output,     // LED1 on PortA.5, set
                                                     // PortA.5 as output
  ...
  GPIOB::MODERPack<GPIOB::MODER::MODER1::Output,  // CS
                   ...
                   GPIOB::MODER::MODER8::Output, GPIOB::MODER::MODER9::Output,
  ...
  // LED2 on PortC.9, LED3 on PortC.8, LED4 on PortC.5 so set PortC.5,8,9 as
  // output PortC.2 sda, PortC.3 scl  for Smbus
  GPIOC::MODERPack<GPIOC::MODER::MODER5::Output, GPIOC::MODER::MODER8::Output,
  ...
  GPIOB::OTYPERPack<GPIOB::OTYPER::OT8::OutputOpenDrain,
  ...
  SPI2::CR1Pack<SPI2::CR1::MSTR::Master,  // SPI2 master
                ...
                SPI2::CR1::DFF::Data8bit, SPI2::CR1::CPOL::High,
                SPI2::CR1::CPHA::Phase2edge, SPI2::CR1::SSM::NssSoftwareEnable,
                SPI2::CR1::SSI::Value1, SPI2::CR1::BR::PclockDiv2,
...
using HardwareTimer5 =
    HardwareTimerBase<TIM5, TimerCountable, InterruptsList<>>;

using Timer =
    DelayTimer<HardwareOverflowTimer<HardwareTimer5, OverflowObservers<>>,
               SysClock, 8>;
...
//
// using AHB1ENRHelper =
// RccEnableHelper<RCC::AHB1ENR,uart::valueAHBENR,spi::valueAHBENR>; using
// APB1ENRHelper =
// RccEnableHelper<RCC::APB1ENR,uart::valueAPB1ENR,spi::valueAPB1ENR>; using
// AppPower = Power<AHB1ENRHelper,APB1ENRHelper>;
...
int main() {
  ...
  //
  //
  // auto temperatureAmbient =
  // Temperature::GetTemperature(Temperature::TemperatureType::Ambient) ;
  ...
  Application::MyDriver::WriteData(
      reinterpret_cast<const std::uint8_t*>(message), strlen(message));
  ...
  LcdDriver::UpdatePartialWindow(Display<400, 300>::image.data(), 0, 0, 400,
                                 300);
  point.y = 100;
  point.x = 130;
  ...
  // LcdDriver::SetPartialWindow(Display<400, 300>::image.data(), 0, 0, 400,
  // 300) ;
  ...
  GPIOC::MODERPack<GPIOC::MODER::MODER15::Output,
                   GPIOC::MODER::MODER14::Analog>::Set();

  result = GPIOC::MODERPack<GPIOC::MODER::MODER15::Output,
                            GPIOC::MODER::MODER14::Analog>::IsSet();
