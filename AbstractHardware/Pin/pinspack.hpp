...
struct PinsPack {
 ...
 private:
  ...
  static_assert(std::is_same<TPins, Collection<Ts...>>::value, "Беда: Одинаковые пины в списке");
  ...
  template <typename Port, typename... Ports>
 ...
 public:
  ...
  template <std::size_t value = 0xffffffffU>
  constexpr static void Set() {
    SetPorts<value>(Ports());
  }

  static void Set(std::size_t value) { SetPorts(Ports(), value); }
  ...
  static void Reset(std::size_t value) { ResetPorts(Ports(), value); }
  ...
  static void Toggle(std::size_t value) { TogglePorts(Ports(), value); }
};
