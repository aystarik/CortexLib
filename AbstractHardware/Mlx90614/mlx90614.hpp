...
struct Mlx90614 {
  enum class Errors { Ok = 0, ReadError = 1, DataError = 2 };
  enum class TemperatureType { Ambient = 0, Object };
  ...
  static std::tuple<float, Errors> GetTemperature(TemperatureType type) {
    ...
    (type == TemperatureType::Ambient) ? Bus::WriteByte(Ram | AmbientTemperature) : Bus::WriteByte(Ram | ObjectTemperature);
    ...
    if ((result >= Msb) || (result == 0U))  // если ошибка
    {
      ...
      // temperature = (temperature >= 273.15f) ? (temperature - ZeroCelsius) :
