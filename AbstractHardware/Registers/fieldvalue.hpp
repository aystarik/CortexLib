...
// Класс для работы с битовыми полями. Добавился тип Base, который необходим для
// того, чтобы проверить, что В регистре устанавливаются те битовые поля,
// которые допустимы для данного регистра template<typename Reg, size_t offset,
// size_t size, typename AccessMode, typename Base, typename Reg::Type value>
