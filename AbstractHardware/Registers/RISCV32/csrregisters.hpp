...
struct CSR {
  ...
  struct MTVT : public CoreRegisterBase<0x307, 32, ReadWriteMode> {
    // using VECTBLADDR = CSR_MTVT_VECTBLADDR_Values<CSR::MTVT, 0, 32,
    // ReadWriteMode, MTVTBase> ; using FieldValues =
    // CSR_MTVT_VECTBLADDR_Values<CSR::MTVT, 0, 0, NoAccess, NoAccess> ;
  };
  template <typename... T>
  ...
  struct MSTATUS : public CoreRegisterBase<0x300, 32, ReadWriteMode> {
    ...
    using XS = CSR_MSTATUS_XS_Values<CSR::MSTATUS, 15, 2, ReadMode,
                                     MSTATUSBase>;  // FIXME for GD is ReadWrite
