...
// Класс для работы с регистром, можно передавать список Битовых полей для
...
class Register {
 public:
  ...
  // Метод Set устанавливает битовые поля, только если регистр может
  ...
  // Метод Write устанавливает битовые поля, только если регистр может
  // использоваться для записи
  template <typename T = AccessMode, class = typename std::enable_if_t<std::is_base_of<WriteMode, T>::value || std::is_base_of<ReadWriteMode, T>::value>>
  static void Write() {
    static constexpr auto value = GetValue();
    *reinterpret_cast<volatile Type*>(address) = value;  // Записываем в регистра новое значение
  }

  // Метод IsSet проверяет что все битовые поля из переданного набора
  // установлены
  template <typename T = AccessMode, class = typename std::enable_if_t<std::is_base_of<ReadMode, T>::value || std::is_base_of<ReadWriteMode, T>::value>>
  static bool IsSet() {
    static constexpr auto mask = GetMask();
    static constexpr auto value = GetValue();
 ...
 private:
  // Вспомогательный метод, возвращает маску для конктретного битового поля на
  // этапе компиляции. Метод определен только в случае, если тип битового поля и
  // базовый тип битового поля для регистра совпадают. Т.е. нельзя устанвоить
  // набор битов не соотвествующих набору для для данного регистра.
  ...
  // Вспомогательный метод, расчитывает общую маску для всего набора битовых
  // полей на этапе компиляции.
  ...
  // Вспомогательный метод, возвращает значение для конктретного битового поля
  // на этапе компиляции. Метод определен только в случае, если тип битового
  // поля и базовый тип битового поля для регистра совпадают. Т.е. нельзя
  // устанвоить набор битов не соотвествующих набору для для данного регистра.
  template <typename T, class = typename std::enable_if_t<std::is_same<FieldValueBaseType, typename T::BaseType>::value>>
  ...
  // Вспомогательный метод, расчитывает значение которое нужно установить в
  // регистре для всего набора битовых полей
  static constexpr auto GetValue() { return (GetIndividualValue<Args>() | ...); }
