...
struct FieldValueBase {
  ...
  // Метод устанавливает значение битового поля, только в случае, если оно
  // достпуно для записи
  template <typename T = typename Field::Access, class = typename std::enable_if_t<std::is_base_of<ReadWriteMode, T>::value>>
  static void Set() {
    RegType newRegValue = *reinterpret_cast<volatile RegType *>(Field::Register::Address);  // Сохраняем текущее значение регистра

    newRegValue &= ~(Field::Mask << Field::Offset);  // Вначале нужно очистить старое
  ...
  //  //Метод устанавливает значение битового поля, только в случае, если оно
  //  достпуно для записи template<typename T = typename Field::Access,
  //    class = typename std::enable_if_t<std::is_base_of<ReadWriteMode,
  //    T>::value>>
  ...
  template <typename T = typename Field::Access,
            class = typename std::enable_if_t<std::is_base_of<ReadMode, T>::value || std::is_base_of<ReadWriteMode, T>::value>>
