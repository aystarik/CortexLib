...
struct HardwareUartBase {
  ...
  template <typename T = Interface, class = typename std::enable_if_t<std::is_base_of<UartTransmit, T>::value>>
  static void EnableTransmit() {
    UartModule::CR1::TE::Enable::Set();
  };

  static void DisableTransmit() { UartModule::CR1::TE::Disable::Set(); };

  template <typename T = Interface, class = typename std::enable_if_t<std::is_base_of<UartReceive, T>::value>>
  static void EnableReceive() {
    UartModule::CR1::RE::Enable::Set();
  };

  static void DisableReceive() { UartModule::CR1::RE::Disable::Set(); };

  template <typename T = Interface, class = typename std::enable_if_t<std::is_base_of<UartTxInterruptable, T>::value>>
  static void EnableTxInterrupt() {
    UartModule::CR1::TXEIE::InterruptWhenTXE::Set();
  };

  template <typename T = Interface, class = typename std::enable_if_t<std::is_base_of<UartRxInterruptable, T>::value>>
  static void EnableRxInterrupt() {
    UartModule::CR1::RXNEIE::InterruptWhenRXNE::Set();
  };

  static void DisableRxInterrupt() { UartModule::CR1::RXNEIE::InterruptInhibited::Set(); };

  static void DisableTxInterrupt() { UartModule::CR1::TXEIE::InterruptInhibited::Set(); };
  ...
  static void DisableTcInterrupt() { UartModule::CR1::TCIE::InterruptInhibited::Set(); };

  template <typename T = Interface, class = typename std::enable_if_t<std::is_base_of<UartInterruptable, T>::value>>
  static void HandleInterrupt() {
    InterruptsList::OnInterrupt();
  }

  static void ClearStatus() { UartModule::SR::Write(0); }
