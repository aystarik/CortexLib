...
struct UartDriver {
  ...
  enum class Status : std::uint8_t { None = 0, Write = 1, WriteComplete = 2, Read = 3, ReadComplete = 4 };
  ...
  //  {
  //    WriteData(reinterpret_cast<const std::uint8_t*>(pString),
  //    strlen(pString)) ; return rOs;
  //  }
