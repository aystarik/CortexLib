...
static constexpr unsigned char EPD_4IN2_lut_vcom0_quick[] = {
    0x00, 0x0E, 0x0E, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static constexpr unsigned char EPD_4IN2_lut_ww_quick[] = {
    0xA0, 0x0E, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static constexpr unsigned char EPD_4IN2_lut_wb_quick[] = {
    0x50, 0x0E, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
};
...
struct ElinkDriver {
  static void SendCommand(ElinkDriverCommands Command) {
    // PinsPack<DcPin, CsPin>::Reset() ;    //DEV_Digital_Write(EPD_DC_PIN, 0);
    // //CsDEV_Digital_Write(EPD_CS_PIN, 0);
    DcPin::Reset();
    CsPin::Reset();
  ...
  static void Init() {
    ...
    SendCommand(ElinkDriverCommands::VcomAndDataIntervalSetting);  // VCOM AND DATA
                                                                   // INTERVAL SETTING
    SendData(0x97);                                                // 97white border 77black border		VBDF 17|D7 VBDW
                                                                   // 97 VBDB 57		VBDF F7 VBDW 77 VBDB 37  VBDR B7

    SetLut();
  }

  static void UpdatePartialWindow(const std::uint8_t *bufferBlack, std::uint16_t x, std::uint16_t y, std::uint16_t width, std::uint16_t height) {
    const std::size_t counts = (width / 8) * height;
  ...
  static void SetPartialMode() { SendCommand(ElinkDriverCommands::PartialIn); }

  static void ClearPartialMode() { SendCommand(ElinkDriverCommands::PartialOut); }
