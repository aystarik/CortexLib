...
#else
#define __LDREX(a) *ptr
#define __CLREX()
#define __STREX(a, b) *ptr
#endif
...
struct AtomicUtils {
  static bool CompareExchange(volatile T* ptr, T oldValue, T newValue) {
    //    // эксклюзивно читаем значение переменной и сравниваем со старым
    //    значением
  ...
  static void Set(T address, T mask, T value, T offset) {
    ...
    } while (!AtomicUtils<T>::CompareExchange(reinterpret_cast<volatile T*>(address), oldRegValue, newRegValue));
  }
};
