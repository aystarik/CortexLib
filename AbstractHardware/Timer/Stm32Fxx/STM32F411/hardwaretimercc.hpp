...
// Этот класс может работать только с Таймером 1 для STM32F411, для него
// прерывание для СС модуля отдельное
template <typename TimerModule, typename TimerList, class = typename std::enable_if_t<std::is_base_of<TIM1, typename TimerModule::Timer>::value>>
...
struct HardwareCCxTimerBase {
  static void HandleInterrupt() {
    ...
    } else if constexpr (std::is_same<CCTimerNum, CC2>::value) {
      if (TimerModule::Timer::SR::CC2IF::InterruptPending::IsSet()) {
        CcTimerObserver::OnCaptureCompare();
      }
    } else if constexpr (std::is_same<CCTimerNum, CC3>::value) {
      // static_assert(TimerModule::Timer::CC3IF)
      if (TimerModule::Timer::SR::CC3IF::InterruptPending::IsSet()) {
        CcTimerObserver::OnCaptureCompare();
      }
    } else if constexpr (std::is_same<CCTimerNum, CC4>::value) {
  ...
  static void Stop() { TimerModule::Stop(); }
};
