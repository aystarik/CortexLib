...
class AssertFlashData {
 public:
  ...
  inline static void Save(const tChar *const file, const tU32 line, const tChar *const message) {
    CriticalSection cs;

    tChar str[maxMsgLength] = "";

    const tS32 status = static_cast<tS32>(snprintf(str,  // lint !e9132
                                                   static_cast<size_t>(maxMsgLength), "%s, %u: %s", file, line, message));
