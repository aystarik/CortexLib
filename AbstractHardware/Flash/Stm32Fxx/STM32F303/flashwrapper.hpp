...
class FlashWrapper {
 public:
  ...
  static void Lock() { FLASH::CR::LOCK::Enable::Set(); }

  static void Erase(const std::size_t addr) {
    //  assert((addr >= SectorsStartAddr) && (addr < (SectorsStartAddr +
    //  SectorsCount * SectorsSize))) ;
    EraseSector(addr);
  }

  template <typename T>
  static void Write(T value, const T *const pDest) {
    // Проверим, что адрес не выходит за границу страницы, предполагаем, что все
    // одной странице
 ...
 private:
  static bool IsReady() { return (FLASH::SR::BSY::NotBusy::IsSet()); }
  ...
  static void EraseSector(const std::size_t addr) {
    ...
    // The software should start checking if the BSY bit equals ‘0’ at least one
    // CPU cycle after setting the STRT bit.
    //  так что просто один дополнительный запрос, чтобы сразу на проверку Busy
    //  не пойти
    FLASH::CR::STRT::Start::Set();
